...
{/*for multiple rows and cols in a seats arrangement create a component */}
...
const bookTickets=async()=>{
  try {
    ...
    const {data}= await axios.post('http://localhost:3000/api/booking/create',{showId: selectedTime.showId,selectedSeats},{headers:{Authorization:`Bearer ${await getToken()}`}});
  ...
  return  show ?(
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
      ...
      <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
          ...
          {/* for center A and B seats */}
          <div className='flex flex-col items-center mt-10 text-xs text-gray-300'>
            <div className='grid grid-cols-2 md:grid-cols-1 gap-8 md:gap-2 mb-6'>
              {groupRows[0].map(row=> renderSeats(row))}
            </div>

            {/* for left and right side seats */}
