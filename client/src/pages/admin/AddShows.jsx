...
0<p className='mt-10 text-lg font-medium'>Now Playing Movies</p>
      <div className='overflow-x-auto pb-4'>
        ...
        {nowPlayingMovies.map((movie)=>(
            <div key={movie._id || movie.id} onClick={()=> setSelectedMovie(movie)} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`}>
                ...
                {/* for adding movie clicked  i mean check box into list */}
      ...
      <div className='mt-8'>
          ...
          <div className='inline-flex items-center gap-2 border border-gray-600 px-3 py-2 rounded-md'>
            ...
            <input min={0} type="number" value={showPrice} onChange={(e)=> setShowPrice(e.target.value)} placeholder='Enter show price' className='outline-none no-spinner'></input>
          </div>
      </div>
...
<div className='mt-6'>
  ...
  <div className='inline-flex gap-5 border border-gray-600 p-1 p1-3 rounded-lg'>
    <input type="datetime-local" value={dateTimeInput} onChange={(e)=> setDateTimeInput(e.target.value)} className='outline-none rounded-md'></input>
    <button onClick={handleDataTimeAdd} className='bg-primary/80 text-white px-3 py-2 text-sm rounded-lg hover:bg-primary cursor-pointer'>Add Time</button>
  </div>
</div>


{/* Display selected times when we click on the ADD time button in add shows page it will display this function */}
...
{/* Add show button for adding shows  */}
