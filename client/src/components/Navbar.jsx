...
<Link onClick={()=>{scrollTo(0,0);setIsOpen(false)}} to="/favorite">Favorites</Link>
      ...
      <div className='flex items-center gap-8'>
        <SearchIcon className='max-md:hidden w-7 h-7 cursor-pointer'/>
        
         
      {
         !user ? (
          <button onClick={openSignIn} className='px-4 py-1 sm:px-7 sm:py-2 bg-primary hover:bg-primary-dull tranisition rounded-full font-medium cursor-pointer'>Login</button>
         ):(
          <UserButton>
